  ...
  useEffect(() => {
    ...
    const delayDebounceFn = setTimeout(async () => {
      ...
      try {
        ...
        const response = await fetch(`http://localhost:5000/api/diseases/search?${queryParams.toString()}`);
  ...
  const handleRowSelect = async (mapping) => {
    ...
    if (mapping.ICD_11_code === 'Not Linked' || mapping.ICD_11_code === '--' || mapping.icdDescription === 'Pending AI Sync...') {
      ...
      try {
        ...
        const response = await fetch('http://localhost:5000/api/diseases/trigger-ai', {
          method: 'POST',
